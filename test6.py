...
inline_bt = types.InlineKeyboardButton(text='Тестовая кнопка', callback_data='bt')
inline_kb.add(inline_bt)
...
async def start(message: types.Message, state: FSMContext):
    """ Отправляет инлайн-кнопку. """
    bot_inlin = await message.answer('инлайн', reply_markup=inline_kb)
    bot_inline_id = bot_inlin.message_id
    
    allUserData = await state.get_data() # загружаем статусы пользователя 
    bot_mes2 = allUserData['bot_mes']
    bot_mes2.append(bot_inline_id)
